public class STint<Value> {
    ...
    private class Node {
        ...
        Node(Integer key, Value val, int N, boolean colour) {
            this.key = key;
            this.val = val;
            this.N = N;
            this.colour = colour;
    ...
    private Iterable<Integer> keys(Node n, Queue<Integer> q) {
        if (n == null) return q;
        else {
            keys(n.left, q);
            q.enqueue(n.key);
    ...
    private boolean contains(Integer key, Node x) {
        if (x == null) return false;
        int cmp = key.compareTo(x.key);
    ...
    public static void main(String[] args) {
        Integer[] ints = {1, 5, 6, 7, 2, 3, 6, 9};
        STint<Integer> st = new STint<Integer>();
